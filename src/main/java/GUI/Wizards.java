...
public class Wizards extends JPanel {
    private final Database dbManager;
    private JTable wizardsTable;

    public Wizards(Database dbManager) {
    ...
    private void initializeUI() {
        ...
        JButton deleteButton = createStyledButton("Удалить покупателя");
        deleteButton.addActionListener(_ -> deleteWizard());
    ...
    private void deleteWizard() {
        int selectedRow = wizardsTable.getSelectedRow();
        int id = (int)(wizardsTable.getModel().getValueAt(selectedRow, 0));
        dbManager.deleteWizard(id);
        loadWizards();
    }
